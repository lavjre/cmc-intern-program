...
func (p *PostgresStorage) GetStats() (*model.AssetStats, error) {
	...
	rowsType, err := p.db.Query(`SELECT type, COUNT(*) FROM assets GROUP BY type`)
	if err != nil {
		return nil, err
	}
	...
	for rowsType.Next() {
		var t string
		var c int
		if err := rowsType.Scan(&t, &c); err != nil {
			return nil, err
		}
		stats.Bytype[t] = c
		stats.Total += c
	...
	for rowsStatus.Next() {
		var s string
		var c int
		if err := rowsStatus.Scan(&s, &c); err != nil {
			return nil, err
		}
		stats.Bystatus[s] = c
	}
...
func (p *PostgresStorage) CountAssets(typeFilter, statusFilter string) (int, error) {
