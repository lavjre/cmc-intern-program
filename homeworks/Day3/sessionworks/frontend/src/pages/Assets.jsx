...
function Assets() {
  ...
  const loadAssets = async () => {
    try {
      setLoading(true);
      setError("");
      const data = await assetsAPI.list({ page: pagination.page, page_size: pagination.page_size, ...filters });
      setAssets(data.data || []);
      setPagination((p) => ({ ...p, total: data.total, total_pages: data.total_pages }));
    } catch (err) {
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case "ip": return <Server size={16} />;
      case "service": return <LinkIcon size={16} />;
      default: return null;
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-description">Manage your external assets (domains, IPs, and services)</p>
      </div>
      ...
      <div className="card">
        {loading ? (
          <div className="loading"><div className="spinner"></div><span>Loading assets...</span></div>
        ) : assets.length === 0 ? (
          <div className="empty-state">
            <Database className="empty-state-icon" size={64} />
            <h3 className="empty-state-title">No assets found</h3>
            <p className="empty-state-description">Get started by adding your first asset</p>
          ...
          <>
            ...
            <div className="pagination">
              ...
              <span className="pagination-info">Page {pagination.page} of {pagination.total_pages} ({pagination.total} total)</span>
              <button className="btn btn-secondary btn-sm" disabled={pagination.page === pagination.total_pages} onClick={() => setPagination((p) => ({ ...p, page: p.page + 1 }))}>
                Next
              </button>
            </div>
          </>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <select className="form-select" value={formData.type} onChange={(e) => setFormData((p) => ({ ...p, type: e.target.value }))} required>
                    <option value="domain">Domain</option>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary">{editingAsset ? "Save Changes" : "Add Asset"}</button>
              </div>
            </form>
          </div>
        </div>
